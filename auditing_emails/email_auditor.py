...
def detect_email_column(df: pd.DataFrame) -> Optional[str]:
    ...
    Looks for common names and checks sample values for email-like patterns.
    """
    ...
    # Fallback: inspect first row for something that looks like an email
    if not df.empty:
        sample = df.iloc[0]
...
class EmailAuditor:
    ...
    def audit_dataframe(
        ...
        Args:
            df: Input DataFrame with at least one email column.
            email_column: Column to use; if None, we auto-detect.
            suppression_emails: Set of exact emails to always drop.
            suppression_domains: Set of domains to always drop.
            progress_callback: Optional function called with (current_index, total_rows)
                for UI progress updates (e.g. Streamlit).
        ...
        for idx, row in df.iterrows():
            ...
            if progress_callback is not None:
                try:
                    ...
                    # Don't break the pipeline for UI issues
                    pass
            ...
            # If we've reached here, keep the row
